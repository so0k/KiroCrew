...
def export_bundle(appearances: Any, pack_id: str) -> dict[str, Any] | None:
    ...
    for slot, entry in animations.items():
        ...
        if not isinstance(content, str) or not content:
            continue
        filename = f"{slot}.{'json' if fmt == 'lottie' else 'svg'}"
    ...
    return {
        ...
        "version": 1,
        "id": ident,
        "manifest": {"meta": meta, "states": states, "sprite": detail.get("sprite") or {}},
        "files": files,
    }
...
def import_bundle(appearances: Any, payload: Any) -> dict[str, Any]:
    ...
    if any(p.get("id") == ident for p in appearances.list_packs()):
