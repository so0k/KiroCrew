        ...
        for i, f in enumerate(findings):
            ...
            for k, v in f.items():
                if not isinstance(v, (str, int, float, bool, type(None))):
                    errs.append(f"findings[{i}].{k} must be a scalar")
            # `line` specifically must be a NUMBER, not merely a scalar. The
            # report redactor used to exempt it on the assumption that it was
            # numeric, and nothing checked: a credential written into `line` as a
            # string rode that exemption to the dashboard. The exemption is gone
            # now, and this keeps the contract honest at the boundary too — the
            # renderer and every consumer treat it as a line number.
            line = f.get("line")
            if line is not None and (isinstance(line, bool)
...
def _read_json_nolink(path: Path, within: Path) -> dict | None:
    """Read and parse a JSON record without following a link planted at `path`.

    Every result file lives in a directory a review worker can reach, and runs are
    concurrent: a prompt-injected worker can replace ANOTHER run's record with a
    symlink, and a plain `read_text` then dereferences it and hands the caller
    attacker-chosen JSON under the victim's change id — the report and the posted
    payload downstream both trust that record.

    `publish_to_shared`/`adopt_from_shared` already guarded their copies this way;
    the readers did not, which left the same hole one function over. Returns None
    when the path is missing, is a plant, exceeds the record cap, or does not parse
    to an object — every caller already treats None as "no record".
    """
    if hooks is None:  # pragma: no cover - standalone fallback
        ...
        raw = hooks.safe_read_file_bytes_nolink(
            str(path), str(within), max_bytes=_RECORD_MAX_BYTES)
    if raw is None:
        return None
    try:
        rec = json.loads(raw)
        ...
        return None
    # A record that is not an object cannot satisfy the contract, and consumers
    # index into it with `.get()`; refuse it here rather than raising at render.
...
def read_result(change_id: str, root: Path | None = None,
    ...
    return _read_json_nolink(path, results_dir(root, run_id))
...
def list_results(root: Path | None = None, run_id: str | None = None) -> list[dict]:
    ...
    for p in sorted(rd.glob("*.json")):
        # Same guard as read_result: one planted link in this dir would otherwise
        # inject a foreign record into every consumer that lists the run.
        rec = _read_json_nolink(p, rd)
        if rec is not None:
            out.append(rec)
    return out
