...
def _redact_deep(value: object, skip: frozenset[str] = frozenset(),
                 _depth: int = 0) -> object:
    """Redact every string inside `value`, at any depth, preserving shape.

    Redacting only top-level strings covered every KEY but not every VALUE: a
    worker that wrote a finding field as `{"evidence": {"k": "<secret>"}}` or
    `["<secret>"]` passed straight through, because `isinstance(v, str)` is False
    for a dict or list, and the nested string reached report.json and the
    dashboard unscrubbed. Walking the structure closes that; `skip` keys are left
    alone at whatever level they appear, and non-string scalars (ints, bools,
    ...
    if isinstance(value, dict):
        return {k: (v if k in skip else _redact_deep(v, skip, _depth + 1))
...
def _redact_deep_map(d: dict, skip: frozenset[str] = frozenset()) -> dict:
    """`_redact_deep` for a dict, typed as a dict so callers keep their shape."""
    return {k: (v if k in skip else _redact_deep(v, skip, 1)) for k, v in d.items()}
