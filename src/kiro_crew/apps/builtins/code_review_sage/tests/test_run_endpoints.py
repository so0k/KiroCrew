...
class TestRunIdSanitization(_RunEndpointBase):
    async def test_traversal_run_id_is_contained(self):
        req = _Req(run_id="../../etc/passwd")
        safe = self.mod._run_id_param(req)
        ...
        rd = store.run_dir(safe)
        self.assertEqual(rd.resolve().parent, store.runs_root().resolve())

    async def test_traversal_delete_cannot_escape(self):
        # A DELETE with a traversal-shaped id resolves to a sanitized (absent) run,
        # so it 404s rather than touching anything outside the runs dir.
        resp = await self.mod._handle_run_delete(
