...
def _claim_changes_under_lock(run: dict, changes: list[str]) -> list[str]:
    ...
    Two filters, both required, applied while ``_CLAIM_LOCK`` is held:
...
async def _run_review_bg(run: dict, changes: list[str]) -> None:
    ...
    Runs are NOT serialized against each other: each one writes into its own
    ``data/runs/<run_id>/`` subtree, and the shared worker pool's semaphore
    (``review.max_concurrent``) is what actually bounds how much review work is in
    flight. Only the claim step is mutually exclusive."""
    run_id = str(run.get("run_id") or "")
    try:
        # Claim the changes this run owns (short critical section — NOT the whole
        # run). Guards against two live runs reviewing/posting the same PR and
        # against re-reviewing a head a just-finished run already delivered.
...
async def _handle_repo_prs(request: web.Request) -> web.Response:
    ...
    try:
        ...
        return web.json_response({"code": "provider_unavailable", "error": str(e)}, status=502)
...
def _run_id_param(request: web.Request) -> str:
    """The ``{run_id}`` path param, sanitized. Path params are user input even
    when the ids we mint are not, and this value reaches the filesystem."""
    return store.safe_run_id((request.match_info.get("run_id") or "").strip())
...
async def _handle_namespaces(request: web.Request) -> web.Response:
    ...
    if request.method == "DELETE":
        ...
        async with _NS_OPS_LOCK:
            ...
            # Prune from active_namespaces first so reviews don't reference a gone
            # ns, then rmtree -- one offloaded helper, since both halves are
            # synchronous file IO that must never run on the event loop, and because
            # a rejected delete must not leave the prune applied on its own.
            def _prune_then_delete() -> dict:
                try:
                    ...
                    logger.debug("could not prune deleted ns from active list",
                                 exc_info=True)
                return learning.delete_namespace(name)

            res = await asyncio.to_thread(_prune_then_delete)
