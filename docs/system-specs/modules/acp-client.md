...
- `"codex"` (`ACP_BACKEND_CODEX`): a Codex ACP agent, authenticated by the user's ChatGPT subscription over OAuth (no API key). Argv comes from `_resolve_codex_acp_argv() -> list[str] | None`, whose ladder accepts **either** distribution form because betting on one would strand installs of the other: `CODEX_ACP_BIN` env override (a non-executable script path is wrapped as `[node, script]`, like the claude resolver), then a standalone `codex-acp` adapter binary (`mise which` → augmented PATH, so a non-login launchd/systemd gateway still finds mise/nvm/fnm/volta shims, the npm global bin, and the cargo bin), then the Codex CLI itself with its ACP subcommand (`["codex", "acp"]`) resolved the same way. The resolution is cached per process; when nothing resolves, `_spawn` raises `AcpError` naming both binaries and the `CODEX_ACP_BIN` override. **Auth preflight:** `codex login` owns the OAuth flow (browser/device, `localhost:1455` callback) and persists `$CODEX_HOME/auth.json` (default `~/.codex/auth.json`, located by `codex_auth_json_path()`); the spawned adapter reads it from disk and Kiro Crew stores no token of its own. A missing file fails **before** spawn with `AcpAuthRequired` carrying the `codex login` remedy (plus the headless options: port-forward 1455 during login, or copy an existing `auth.json` to the host), and an expired token — which kills the adapter mid-handshake with a `not logged in` banner on **stderr** rather than a typed error — is re-raised as the same `AcpAuthRequired` when `ensure_ready` gives up (the shared not-logged-in stderr classifier), instead of surfacing as a generic startup failure. **Dialect:** codex speaks the public ACP spec, so it shares the spec-adapter sites with claude (keyed on `_is_spec_adapter`): integer `protocolVersion` `1`, no `session/set_mode`, model applied via `session/set_config_option` and only when the session advertised a `model` config option (an adapter build without it would reject the call with -32602 and kill startup for a setting it cannot honor, so the session stays on the backend default), and MCP servers passed in the `session/new` / `session/load` params. The `model` guard engages on **advertised options present but no `model` entry among them**: `supports_config_option` returns True for an EMPTY `_acp_config_options`, so an adapter that has advertised nothing yet still gets the push and can answer -32602. That direction is deliberate — a backend advertising options lazily (after the first turn) would otherwise ignore the operator's pick permanently, which is worse than one recoverable wire rejection. Kiro-dialect sites (`set_mode`, the `~/.kiro/sessions/cli/<sid>.json` resume check, the transcript jsonl seek, entitlement checks against the kiro-advertised model set, the `is_kiro_cli` sandbox flag) key on `_is_kiro` and are skipped. Because codex reads no kiro agent config, `_codex_session_mcp_servers()` injects the managed `kirocrew-core` / `kirocrew-cron` / computer servers — reshaped from their invocation form to the spec stdio entry `{name, command, args, env}`, env as `[{name, value}]` — or the session would have no cron, memory, or core tools. There is deliberately **no `type` key**: in the ACP schema `type` tags the http/sse `McpServer` variants and stdio is the untagged default, so a strict (Rust serde) deserializer would reject the extra field and fail the whole `session/new`, leaving the session with zero managed tools — the exact outcome the injection exists to prevent. `env` carries `agent._managed_mcp_env()`, i.e. the `KIROCREW_HOME` pin under an override data home: a child does **not** inherit the override and the entry's `env` is the only channel, so omitting it would have the shims read the DEFAULT home while the gateway writes the override one. Session identity needs no entry of its own (`KIROCREW_SESSION_KEY` is on the spawn env, which the adapter's children inherit). A server whose invocation cannot be resolved is dropped with a warning rather than failing the session. `_session_mcp_servers()` then merges that list with the MCP-gateway broker stubs (`_pooled_mcp_servers`) that both backends already declare, **deduped by name with the broker stub winning**: the rewriter wraps every stdio server in the materialized agent spec, so all three managed names appear on both sides, two elements with one name is undefined in the ACP schema, and the stub reaches the same backend through gatewayd (spawned with the gateway's own env, so the `KIROCREW_HOME` pin still holds) while also being the addressing layer MCP Apps callbacks route through. On a spec adapter each surviving element is then reduced to `_ACP_STDIO_SERVER_KEYS` = `{name, command, args, env}`, because `mcp_gateway.session_servers._acp_server_entry` deliberately preserves kiro-cli passthrough keys (`autoApprove`, `timeout`, `disabledTools`, operator vendor keys) that the same strict deserializer would reject. The kiro path is unchanged by the merge: its alt list is empty and pooled names are already unique, so it is the identity. **Auth-required is non-retryable:** the missing-`auth.json` `AcpAuthRequired` raised inside `_spawn` bypasses `ensure_ready`'s one-shot retry ladder rather than paying a second full spawn to re-raise the same error. Claude-only behavior (`settings.local.json` seed, `_meta.claudeCode`, `CLAUDE_CODE_EXECUTABLE`, the substitution-advisory retry) stays keyed on `_is_claude`.

**Composite model ids (codex).** codex-acp is inconsistent between its two directions, verified live against the running adapter: it **advertises** one `availableModels` entry per effort level — `model_name` `gpt-5.6-sol[low]` … `gpt-5.6-sol[ultra]`, `display_name` `GPT-5.6-Sol (low)` … — while the `configId="model"` push **accepts only the bare id** (`{"model": "gpt-5.6-sol"}` was acked `{"ok": true}` on the same session that had just answered `gpt-5.6-sol[medium]` with -32602 invalid params). `GET /api/models` serves those rows and the picker POSTs one verbatim, so every pick arrives composite: the split below is the bridge over an upstream inconsistency (worth filing upstream — the adapter should either accept its own advertised spelling or advertise the bare id), not a shape we chose. `codex_base_model_id()` takes the suffix off in `set_model`, `_apply_startup_model`, `_capture_available_models`' `currentModelId` record (so `served_model` and the usage rows never carry a composite), and the dashboard's `_wire_model_id`, recording the bare id so the startup re-apply cannot resurrect the composite: the stored model is re-applied at EVERY session init, so one composite fails every later handshake, not just the one call. The suffix is **routed, not discarded** — the advertised entries for one model differ ONLY by effort, so it carries the user's effort pick. `codex_model_effort_suffix()` hands it to the effort mechanism: `_adopt_codex_effort_pick` stores it as the slot's `reasoning_effort` once the model half has landed, so `_reapply_effort_after_live_switch` pushes it live on the `effort` option and the persisted slot value re-applies it at the next cold start via `AcpProvider._apply_initial_effort`; `_apply_codex_startup_effort` covers a composite arriving from config at startup. Both validate against the levels THIS session advertised rather than the canonical five, because codex offers an extra `ultra`, and the startup push is deliberately ordered BEFORE `_apply_initial_effort` so an explicit slot/config level still wins. A pick with no suffix leaves effort untouched: on a spec adapter `clear_effort()` returns False having touched no wire, and the model runs at its own default effort. Because the accepted value is bare while the advertised set is composite, `model_is_unusable()` falls back to comparing **bases** when the advertised set has that shape (`_advertised_is_codex_composite`: every entry suffixed over a non-empty base, and some base under two different suffixes) — otherwise a slot holding the bare id the adapter itself accepted would be called unentitled by the warm-pool post-claim check. The shape gate is what keeps kiro/claude on exact match, where `[1m]` marks the 1M-token window and distinguishes real entitlements; an id whose base is unknown still fails the check. Finally a model the adapter refuses at startup is a warning, not a dead handshake: `_apply_codex_startup_model` catches `AcpError`, logs the refused value and leaves the session on the backend default (recorded as `_model` so the warm-pool re-apply stops re-offering it), which makes a bad persisted model self-healing instead of fatal on every respawn. An EXPLICIT live pick keeps the opposite contract — it surfaces the error to the user rather than swapping models silently. The split is codex-scoped by design.

**Approval-gate boundary (codex).** Kiro Crew's PreToolUse gate — the builtin denied-command rules, the `~/.aws` / `~/.ssh` sensitive-path block, and the governance ceiling, all evaluated by `hooks.on_tool_call` — hangs off the adapter sending `session/request_permission`. kiro-cli is made to ask by `--agent <spec>` and its `allowedTools`; claude-agent-acp by the seeded `defaultMode: default`. A codex adapter has neither: whether it asks is decided by its **own** `approval_policy` in `$CODEX_HOME/config.toml`, which Kiro Crew reads (`codex_approval_policy()`) and never writes. Under `never` or `on-failure` (`CODEX_PERMISSION_BYPASS_POLICIES`) the adapter auto-approves commands inside its workspace sandbox without asking, so those tool calls never reach the gate; `untrusted` and `on-request` do ask. This is surfaced, not silently enforced: `_spawn` logs a warning naming the policy and the config path, and `kirocrew doctor`'s **Codex Backend** section carries an `approvals:` row (⚠️ + an `issues` entry for a bypassing policy, ⚠️ without one when the value is not determinable — absent file, absent key, or no TOML parser on a 3.10 interpreter — and ✅ otherwise). It is not a hard refusal because the value read is the top-level one and a `[profiles.*]` selection inside the adapter can override it, which would make a refusal wrong in both directions. That same caveat is **named in the healthy row too**: it prints `approval_policy=<value> (top-level)` and states that a `[profiles.*]` selection can override it and is not resolved here. The uncertainty is identical in both directions, so an unqualified ✅ would tell an operator the denied-command rules and the governance ceiling are being consulted when a profile may have turned asking off.

**MCP dispatch classification (codex).** codex-acp stamps its MCP tool calls `kind: "execute"` — the shell kind — with the adapter-authored `_meta.is_mcp_tool_call: true` and the real dispatch target in `rawInput` `{server, tool, arguments}`. `spec_adapter_mcp_identity()` (`acp/_dispatch.py`) recognises that shape and reclassifies the call: `is_shell=False`, and the `{server, tool}` pair feeds the same trusted-identity plumbing as kiro-cli's `_meta.kiro.mcpServerName`/`toolName` (`AcpEvent.mcp_server_name`/`tool_name`, plus the per-`toolCallId` caches the later `session/request_permission` inherits from), so `hooks.on_tool_call` governs the canonical `mcp__<server>__<tool>` through the normal approval pipeline. Without the reclassification the shell deny-by-default blocks **every** spec-adapter MCP call: the permission gate sees `is_shell=True` with no recoverable `command` (the rawInput is the MCP triple, not a command line) and refuses. Fail-closed direction is preserved: a server/tool-shaped `rawInput` **without** the `_meta` flag keeps its shell classification (rawInput alone is model-authored), and when both identity channels are present the kiro `_meta.kiro` channel wins. Applied at all four classification sites — initial tool_call and kind-carrying refinement, in both `AcpClient` and the shared `_dispatch` builders.

**Permission-title fallback (codex).** codex-acp's `session/request_permission` payload carries only `{toolCallId, kind, status}` under `toolCall` — **no `title`** — so every codex approval would be named `"unknown"`. That is not a cosmetic defect: the dashboard's trust dropdown records `event.title` as the trust PATTERN (`perm_meta["tool_title"]`, matched by `_matches_trusted_pattern`), so one "always allow" on an `unknown` dialog blanket-auto-approves **every** later permission request in the session — observed live. `_dispatch.resolve_permission_title()` is the single rule both transports call: a payload-provided title always wins (the kiro dialect sends one, so that path keeps its own name — normalised only, surrounding whitespace stripped and a non-string title treated as absent); a missing, empty, or literally-`"unknown"` title resolves to the canonical `mcp__<server>__<tool>` when BOTH identity halves are cached, else to the title the preceding `tool_call` cached, else stays `"unknown"`. The identity is preferred because it is adapter-authored (`_meta.kiro`, or the `_meta.is_mcp_tool_call`-guarded rawInput triple) rather than LLM prose, which makes it the safest value to persist as a trust pattern and the same key `hooks.on_tool_call` governs; half an identity is not used, since a fabricated canonical name would be trusted as a tool that does not exist. The fallback title rides a new per-`toolCallId` cache (`AcpClient._tool_call_titles` / `AcpSessionHandle._tool_call_titles`, threaded into the shared builders as `title_cache`) with exactly the lifecycle of the `is_shell` cache: written from the initial `tool_call`, refreshed by a refinement only when that refinement carries a title of its own, cleared per turn alongside its siblings. What it holds is the **adapter-authored `title`** (the literal invocation), redacted — deliberately NOT the pill's display title, which `select_tool_title` takes from the model's own `rawInput.description` when present: the resolved name is what `hooks.on_tool_call` matches against `auto_approve_tools`, so caching the description would let the model label `chmod -R 777 …` as an `ls` the user has trusted, and a description-only refinement would overwrite the real invocation. The pill still shows the description; only the gate's name is adapter-authored. A `toolCallId` no `tool_call` was seen for resolves to `"unknown"` — nothing is invented on a cache miss — and that placeholder can never become a trust key: `chat_handlers._record_trusted_pattern()` refuses `UNRESOLVED_TOOL_TITLE` (and the `unknown *` / bare form the base-command dropdown derives from it), so an unresolvable request still goes to the human instead of granting a session-wide wildcard.

**MCP server-name sanitizer (codex).** codex accepts an MCP server name matching `[a-zA-Z0-9_-]+` **only**, and DROPS an entry that fails it silently: no wire error, no log line, the server just never appears in the tool registry. Proven live in one session where `aws-docs` was listed and searchable while `awslabs.aws-documentation-mcp-server`, declared in the same `session/new` (dots being the disqualifier), was absent. `_session_mcp_servers()` therefore resolves the whole session's names through `_codex_wire_mcp_names()` on the `_is_codex` branch — codex only, not every spec adapter: the charset rule is codex's own, and renaming on a backend that accepts the name would break its per-server rules for nothing. Legality is tested with `fullmatch`, because a `$`-anchored `match` also accepts a trailing newline that codex's own end-of-text `$` rejects. Resolution is **two-pass, so the outcome never depends on declaration order**: every already-legal name claims its own spelling first, then each remaining name is sanitized (runs of invalid characters collapse to one `-`, leading/trailing `-` stripped, an all-invalid name falls back to a `sha256` prefix). A sanitized result that collides — with a name claimed in pass 1, with another name reducing to the same base, or with a *reserved* name — takes a 6-hex-char `sha256` suffix of the original, and when two illegal names contest one base **both** take the suffix, so which one would have kept the plain spelling is not an ordering accident either. Collapsing a collision would drop a declared server's whole toolset, which is the very failure the sanitizer exists to prevent, so a legal name is never displaced by a sanitized one. The reserved set is the managed `kirocrew-*` servers (`_reserved_codex_mcp_names()`): `kirocrew-core` and `kirocrew-computer` are trusted downstream by literal spelling (the session-directive server check, `CU_MCP_SERVER`), and a managed entry can be absent from a session when its invocation fails to resolve, so reserving the names keeps a configured `kirocrew core` from ever speaking under one. Every rename emits a `logger.warning` naming declared → wire, once per name. Deduping keys on the wire name, so one server declared by both sources still collapses to a single element (broker stub winning) while two different names that sanitize onto one stay two servers. **The rename is a codex-side label, not a new identity:** codex reports the WIRE name back as `rawInput.server`, so `_session_mcp_servers()` records every rewrite in `_mcp_wire_to_declared` and `_declared_mcp_server()` translates it back where the tool_call identity is captured. `AcpEvent.mcp_server_name`, the per-`toolCallId` caches the permission event inherits, and the governed canonical `mcp__<server>__<tool>` therefore all carry the DECLARED name — a per-server governance rule, an app-own-server match (`hooks._app_owns_mcp_server` partitions on `:`, a character no legal codex name can hold), a trust pattern and the session-directive check keep matching what the operator wrote. Routing is likewise unaffected: the gateway broker stub keeps its own `--server <original>` argv and a managed entry keeps its command. The kiro path is untouched — kiro-cli accepts a dotted name, no name is rewritten, and the merge is identity. `AcpRuntime.create_session` passes its `mcpServers` array out verbatim, with neither this guard nor the `_ACP_STDIO_SERVER_KEYS` reduction: that transport speaks the kiro dialect only, and a comment at the call site names both guards a spec-adapter path would have to adopt.

**`$CODEX_HOME/auth.json` is a protected path.** Selecting the codex backend makes that file load-bearing, and it holds a live ChatGPT-subscription OAuth access + refresh pair, so the leaf `.codex/auth.json` is on `security._SENSITIVE_HOME_DIRS`. The leaf only — `$CODEX_HOME` also holds `config.toml` (which an operator debugging the approval row legitimately reads) and session logs. One entry there arms both the fs_read/fs_write gate (`is_sensitive_path`) and the bash matcher (`_build_sensitive_regex`, its verb-independent catch-all, and `_RELATIVE_SENSITIVE_RE`), which covers cat/head/base64/cp/python-open and any novel verb — wider than per-verb `DeniedCommandRule` entries would be. The adapter is unaffected: it reads the file in-process, never through the gate, and the sandbox path-hiding lists (`sandbox._STANDARD_DIRS` / `_CC_DIRS` / `_STRICT_DIRS`) are a separate set, so the spawn still sees it. **What that protection is worth is bounded by the approval-gate boundary above**, and the claim must not be read as stronger: both matchers are reached only through `hooks.on_tool_call`, which fires only on a `session/request_permission`, so the entry protects `auth.json` under `approval_policy` `untrusted` / `on-request` and **not** under the bypass policies `never` / `on-failure` (`CODEX_PERMISSION_BYPASS_POLICIES`), where the adapter auto-approves its sandboxed commands and a read it performs there is never offered to the gate. No sandbox tier backs it up either — `.codex` is on none of the path-hiding lists by necessity, since the adapter must read `auth.json` to authenticate and a tool call is its child in the same sandbox, so nothing can hide the file from the agent while keeping it visible to the adapter. Scope limit: home-anchored like every entry on that list, so a `$CODEX_HOME` relocated outside `$HOME` is not covered.
...
**`clientCapabilities` in the `initialize` request.** Both transports (`AcpClient._initialize_session` and `AcpRuntime`) send the shared `ACP_CLIENT_CAPABILITIES` dict from `acp/types.py`. Previously the key was omitted entirely, so the agent assumed the all-false default. **Spec adapters send `ACP_CLIENT_CAPABILITIES_SPEC_ADAPTER` instead** — the same dict minus `elicitation` (see that row for why).
...
| `elicitation` | `{form: {}, url: {}}` | **Forward-bet.** kiro-cli 2.14.0 compiles the `elicitation/create` schema (form + url modes, `requestedSchema` with `enum`/`oneOf` single-select and array multi-select) and gates it on this capability, but does **not** yet route an MCP server's `elicitation/create` out over ACP — a stub MCP server issuing one gets `-32601 method not found`. Declaring it costs nothing today and makes the richer native prompt available the moment upstream ships the bridge. **Consequence to accept:** once the bridge lands, inbound `elicitation/create` requests will be rejected by `_reject_unknown_server_request` until a handler is wired — the same failure mode as today, but then attributable to us rather than upstream. **Kiro-cli dialect only.** On the spec dialect this bet already came due: codex-acp gates MCP tool-call approvals on `elicitation.form` and, when declared, sends every approval as `elicitation/create`; the rejection is converted to `action: "cancel"`, silently cancelling every MCP tool call in the session. Spec adapters therefore send `ACP_CLIENT_CAPABILITIES_SPEC_ADAPTER` (this dict minus `elicitation`), which makes codex-acp fall back to `session/request_permission` — fully served by the normal approval pipeline. Do not re-add `elicitation` for spec adapters without shipping an `elicitation/create` handler first. |
...
- **Poll-driven spawn sites are readiness-gated.** `kiro-cli` auto-launches an
  ...
  **Backend scope:** every probe behind that latch asks about `kiro-cli` — its
  binary, its config, its sign-in — so `reject_if_kiro_unverified()` opens
  unconditionally when `agent.acp_backend` (read via
  `acp.client.configured_acp_backend()`, off the loop) selects another adapter.
  On such a host the latch is a certain `ready=False` and the gate would 503
  resume, regenerate, rewind, `/api/sessions/usage` and
  `POST /v1/chat/completions` forever while ordinary sends — deliberately
  ungated — kept working; that partial failure is worse than no gate. The
  selected adapter's own auth surfaces as an `AcpAuthRequired` error card, and
  its readiness is reported up front by `kirocrew doctor`. `/api/models` never
  reaches the gate on that path at all: it serves the live session's advertised
  models instead of shelling out (see the codex backend bullet).
