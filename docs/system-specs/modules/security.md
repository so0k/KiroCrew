...
- Sensitive paths: `~/.aws`, `~/.ssh`, `~/.gnupg`, `~/.gpg`, `~/.config/gcloud`, `~/.azure`, `~/.docker/config.json`, `~/.kube/config`, `~/.npmrc`, `~/.pypirc`, `~/.netrc`, `~/.git-credentials`, `~/.codex/auth.json`, `~/.kiro/crew/.env`, `~/.kiro/crew/sel_hmac.key`, `~/.kiro/crew/trust`, `~/.kiro/crew/security_events.jsonl`, `~/.kiro/crew/app_admission.json`, `~/.kiro/crew/run`
- **Codex OAuth store (`~/.codex/auth.json`)** — `codex login` persists a live ChatGPT-subscription OAuth access + refresh token pair there, and selecting the codex ACP backend (`agent.acp_backend`, see [acp-client.md](acp-client.md)) makes it load-bearing. It is the **leaf only**, deliberately not the `.codex` directory: `$CODEX_HOME` also holds `config.toml` (whose `approval_policy` an operator debugging the PreToolUse-gate row legitimately reads) and session logs, and blocking those buys no security. The single `_SENSITIVE_HOME_DIRS` entry arms both the tool path (`is_sensitive_path`) and every shell form (`is_sensitive_bash_command` — the verb-anchored branch, the verb-independent catch-all, and `_RELATIVE_SENSITIVE_RE`), which is wider than per-verb `DeniedCommandRule` entries and is the shared-matcher route the write-protected-leaf scope note prescribes. The Codex adapter is unaffected: it reads the file in-process, never through this gate, and the sandbox path-hiding lists (`sandbox._STANDARD_DIRS` / `_CC_DIRS` / `_STRICT_DIRS`) are a separate set, so the spawn still sees it. **Enforcement scope — this is a gate check, not a filesystem control, and on a codex host the adapter's approval policy decides whether it is reached at all.** Both matchers are consulted only by `hooks.on_tool_call`, and that PreToolUse gate runs only when the adapter sends `session/request_permission`. Which codex sessions ask is decided by the adapter's own `approval_policy` (the approval-gate boundary in [acp-client.md](acp-client.md)): under `untrusted` / `on-request` the entry protects the file, while under the bypass policies `never` / `on-failure` (`CODEX_PERMISSION_BYPASS_POLICIES`) the adapter auto-approves commands inside its sandbox without asking, so a `cat ~/.codex/auth.json` it runs there reaches neither matcher. **No OS-sandbox tier closes that gap**, and none can: `.codex` is deliberately absent from every path-hiding list because the adapter process must read `auth.json` to authenticate at all, and a tool call runs as its child inside the same sandbox — there is no tier that hides the file from the agent while leaving it visible to the adapter serving it. The protection is therefore exactly as strong as the operator's `approval_policy`, which is why a bypassing value is a `kirocrew doctor` issue and a spawn-time warning rather than a silent default. Scope limit: home-anchored like every entry here, so a `$CODEX_HOME` relocated outside `$HOME` is not covered — that list has no re-anchoring hook except the crew data home's.
