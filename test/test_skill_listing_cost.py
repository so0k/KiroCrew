...
class TestSizeAndMatches:
    ...
    def test_matches_is_none_without_a_ledger_entry(self, tmp_path: Path) -> None:
        """None is not zero — an entry can also age out of the window."""
        _write(tmp_path / "skills", "fresh")
        loader = SkillsLoader(skills_path=tmp_path / "skills", install_builtins=False)
        assert _one(loader, "fresh")["matches"] is None

    def test_matches_reflects_recorded_trigger_hits(self, tmp_path: Path) -> None:
        ...
        assert _one(loader, "hot")["matches"] == 3
