 ...
 *   Writes raw.webm; convert to mp4/gif with ffmpeg.
 */
...
async function main() {
  ...
  // Name the real failure. Playwright writes the video on context.close(), so
  // an empty dir means the recording never started — and indexing straight
  // into join() would report that as an unrelated TypeError on `undefined`.
  if (!webm) throw new Error(`no .webm written to ${OUT} — the recording did not start`)
  renameSync(join(OUT, webm), join(OUT, 'raw.webm'))
  console.log(`wrote ${join(OUT, 'raw.webm')}`)
}
