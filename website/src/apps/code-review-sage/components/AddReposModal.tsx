                    ...
                    <ul className="flex flex-col gap-1.5 list-none p-0 m-0">
                      {recentRows.map((r) => (
                        <RepoRow
                          ...
                          meta={`${relativeAge(r.last_contributed_at)} · ${r.contribution_count} event${r.contribution_count === 1 ? '' : 's'}`}
                ...
                {mineRows.length > 0 && (
                  <>
                    <SectionHead
                      ...
                      hint={mine?.truncated ? 'newest 100 by push date' : undefined}
                    />
                    <ul className="flex flex-col gap-1.5 list-none p-0 m-0">
                      {mineRows.map((r) => (
                        <RepoRow
                          key={r.full_name}
                          fullName={r.full_name}
                          meta={r.pushed_at ? `pushed ${relativeAge(r.pushed_at)}` : undefined}
                          pinned={isPinned(r.full_name)}
                          onPin={() => pinRepo(r.owner, r.repo)}
                          badges={(
