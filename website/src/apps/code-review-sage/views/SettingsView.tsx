          ...
          <div className="mt-6 rounded-xl border border-border bg-card px-4 py-4">
            ...
            >
              <select
                ...
                value={s.model ?? ''}
                onChange={(e) => saveMut.mutate({ model: e.target.value || null })}
                className={SELECT_CLASS}
              >
                <option value="">{i18nT('apps.codeReviewSage.views.settingsView.default_agent_config')}</option>
            ...
            >
              <select
                ...
                value={s.effort ?? ''}
                onChange={(e) => saveMut.mutate({ effort: e.target.value })}
                className={SELECT_CLASS}
              >
                <option value="">{i18nT('apps.codeReviewSage.views.settingsView.default_model_provider')}</option>
                {(data.efforts ?? []).map((ef) => <option key={ef} value={ef}>{ef}</option>)}
              </select>
            </Field>
            ...
            >
              <select
                ...
                value={s.max_concurrent ?? 5}
                onChange={(e) => saveMut.mutate({ max_concurrent: Number(e.target.value) })}
                className={SELECT_CLASS}
              >
                {Array.from({ length: data.max_concurrent_max ?? 30 }, (_, i) => i + 1)
