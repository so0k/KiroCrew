...
function DetailPanel({ detail, isActive, onClose, onApply, onExport, onEdit, onDelete, colorMap }: {
  ...
  useEffect(() => {
    ...
    api?.getCrewCompanionConfig?.().then((c) => {
      if (typeof c?.accessory === 'string') setAccessory(c.accessory as GhostAccessory)
    }).catch(() => {})
  }, [isDefaultKiro])
    ...
    <div role="presentation" style={S.detailOverlay} onClick={onClose}>
      <div role="dialog" aria-modal="true" style={S.detailPanel} onClick={(e) => e.stopPropagation()}>
        ...
        {isDefaultKiro && (
          ...
          >
            <span style={{ transition: 'transform 200ms', transform: showColorCustomizer ? 'rotate(180deg)' : 'none', display: 'inline-block' }}>▼</span>
      ...
      <SpriteImporter
        ...
        onDone={async (result: SpriteImportResult) => {
          ...
          if (res?.ok) {
            ...
            setError(res?.error || i18nT('apps.crewCompanion.gallery.spritePackFailed'))
          }
          setMode('gallery')
        }}
