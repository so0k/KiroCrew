...
export const SpriteImporter: React.FC<Props> = ({ existingPack, onDone, onCancel }) => {
  const { i18nT } = useLang()
  ...
  const S = {
    ...
    select: { background: 'var(--cc-input-bg)', border: '1px solid var(--border)', borderRadius: 6, padding: '4px 8px', color: 'var(--text)', fontSize: 12, outline: 'none' },
                ...
                return (
                  <div key={s} style={{
                    ...
                    <div style={{ width: 64, height: 64, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', transform: flipX ? 'scaleX(-1)' : 'none' }}>
                      {row ? <SpriteRenderer src={row.dataUri} frameWidth={frameW} frameHeight={frameH} fps={fps} displaySize={64} /> : <span style={{ fontSize: 24, color: 'var(--text-faint)' }}>＋</span>}
                    </div>
                    <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text)' }}>{s} *</span>
                    <select value={rowIdx ?? ''} onChange={e => setAssignments(prev => ({ ...prev, [s]: e.target.value === '' ? null : Number(e.target.value) }))} style={{ ...S.select, width: '100%', fontSize: 10 }}>
                      <option value="">—</option>
                      {rows.map(r => <option key={r.index} value={r.index}>{i18nT('apps.crewCompanion.sprite.row', { n: r.index + 1 })}</option>)}
                    </select>
                  </div>
                )
              })}
