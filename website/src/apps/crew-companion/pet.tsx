...
function Companion() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const r = await fetch(`${PENDING_PATH}?since=${cursorRef.current}`, {
          credentials: 'same-origin',
        })
        if (!r.ok) return
        const data = (await r.json()) as { cursor: number; fires: Fire[] }
        if (stopped) return
        ...
        // Only the NEWEST is shown. Stacking bubbles creates a queue of things to
        // dismiss, which reads as obligation rather than company.
        const latest = data.fires[data.fires.length - 1]
        if (!latest) return
        // A command is not something to say; it was handled above.
        if (latest.kind === 'command') return
        ...
        if (latest.key) {
          ...
          // silent beats showing someone the string "break.water.3".
          if (!phrase) return
          text = phrase
        }
