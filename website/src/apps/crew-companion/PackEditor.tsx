...
function SlotThumbnail({ data }: { data: SlotData | null }) {
  if (!data) {
    return <div style={S.slotPlaceholder}>＋</div>
  }
...
export const PackEditor: React.FC<PackEditorProps> = ({ existingPack, onSave, onCancel }) => {
  ...
  const doSave = useCallback(async (asNew: boolean) => {
    ...
    try {
      ...
      // Open-ended random extras → { name: content }
      const randomData: Record<string, string> = {}
      for (const x of extras) {
        const nm = x.name.trim()
        const slot = slots[x.id]
        if (nm && slot?.content) randomData[nm] = entryContent(slot)
      }
    ...
    <div style={S.root}>
      ...
      */}
      <div style={S.titleBar}>
        ...
        >← {i18nT('apps.crewCompanion.editor.back')}</button>
      </div>
      <PackInfoHeader
      ...
      <div style={S.body}>
        ...
        <div style={S.grid}>
          ...
          >
            <div style={{ fontSize: 20, color: 'var(--text-muted)', lineHeight: 1 }}>＋</div>
