...
const canControlInjection = (s: Skill) => s.source === 'kirocrew' && !s.always

/** Compact byte size, e.g. `21 kB`. Sub-kB stays in bytes so a tiny skill does
 *  not render as a misleading `0 kB`. */
function formatBytes(n: number): string {
  return n < 1024 ? `${n} B` : `${Math.round(n / 1024)} kB`
}

/** Compact character count for the cost figure, e.g. `13.4 M`. */
...
function InjectionRow({ skill }: { skill: Skill }) {
  ...
  const matches = skill.matches ?? null
  const spent = matches !== null && size ? matches * size : null
  ...
  return (
    <div className="px-4 py-2.5 border-b border-border shrink-0">
      ...
      <div className="mt-2 text-[11px] text-muted font-mono">
        {matches === null
          ? i18nT('pages.overview.skillsTab.size_no_matches', { size: formatBytes(size) })
          : i18nT('pages.overview.skillsTab.cost_line', {
              size: formatBytes(size),
              matches: String(matches),
              chars: formatChars(spent ?? 0),
            })}
      </div>
