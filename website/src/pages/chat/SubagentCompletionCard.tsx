...
const SubagentCompletionCard = memo(function SubagentCompletionCard({
  ...
  // A failure opens expanded. The header can only say THAT something failed; the
  // reason is the reader's next question, and a digest already orders failures
  // first for exactly this reason. Successes stay folded — their payload is a
  // result path, not something to read.
  const [expanded, setExpanded] = useRowDisclosure(disclosureKey, failed)
