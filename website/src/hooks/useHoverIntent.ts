...
export function useHoverIntent(options: Options = {}): HoverIntent {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      // stopPropagation because this listener is on `document`: without it one
      // Escape dismisses the surface AND whatever else the page happens to
      // treat as dismissable, silently clearing a card the user never aimed at.
      e.stopPropagation()
      close()
