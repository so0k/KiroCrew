...
describe('ghost accessory layer', () => {
  ...
  it('loads the asset-backed props from files instead of redrawing them', () => {
    for (const id of ASSET_BACKED) {
      ...
      expect(src, `"${id}" has no src`).not.toBe('')
      // A data: URI here means the art was inlined by hand rather than bundled —
      // which is how the stand-in art got in last time.
      expect(src.startsWith('data:'), `"${id}" inlines art instead of loading it`).toBe(false)
      unmount()
    }
  })
