...
describe('Code Review Sage shell', () => {
  ...
  it('puts the PR content behind tabs, with Sage Review leading', async () => {
    ...
    const reviewTab = await screen.findByRole('tab', { name: /Sage Review/i })
    expect(reviewTab.getAttribute('aria-selected')).toBe('true')
    for (const name of [/Description/i, /Comments/i, /Checks/i]) {
      expect(await screen.findByRole('tab', { name })).toBeTruthy()
    }

    // Description is NOT rendered until its tab is chosen (the point of tabs:
    // a long description used to push the review below the fold).
    expect(screen.queryByText(/Caps the jar/)).toBeNull()
    await userEvent.click(screen.getByRole('tab', { name: /Description/i }))
  ...
  it('shares ONE provider call across all four tabs', async () => {
    // Switching tabs must not re-hit the provider: that is why the source is a
    // single query rather than one per tab.
    ...
    await screen.findByRole('tab', { name: /Sage Review/i })
    await userEvent.click(screen.getByRole('tab', { name: /Description/i }))
    await userEvent.click(screen.getByRole('tab', { name: /Comments/i }))
    await userEvent.click(screen.getByRole('tab', { name: /^Checks/i }))
  ...
  it('opens a single-PR run WITH its PR context, not a bare run view', async () => {
    ...
    // Provider-backed context tabs are present even though the PR was never in
    // the loaded list — the reference is derived from the run's change URL.
    for (const name of [/Sage Review/i, /Description/i, /Comments/i, /^Checks/i]) {
      expect(await screen.findByRole('tab', { name })).toBeTruthy()
    }
    await userEvent.click(screen.getByRole('tab', { name: /Description/i }))
    expect(await screen.findByText(/Caps the jar/)).toBeTruthy()
...
describe('posting a review to the pull request', () => {
  ...
  it('offers to post, labelled with the comment count', async () => {
    ...
    expect(await screen.findByRole('button', { name: /Post 2 comments/ })).toBeTruthy()
  })
  ...
  it('asks before writing to someone else\'s pull request', async () => {
    ...
    expect(screen.getByText(/Post 2 comments to the pull request\?/)).toBeTruthy()
  })
  ...
  it('posts only THIS pull request when the run covers several', async () => {
    ...
    await userEvent.click(await screen.findByRole('button', { name: /Post \d+ comment/ }))
    await userEvent.click(screen.getByRole('button', { name: /^Post$/ }))
  ...
  it('shows what was posted instead of offering again', async () => {
    ...
    expect(await screen.findByText(/Posted 2 comments/)).toBeTruthy()
    expect(screen.queryByRole('button', { name: /Post 2 comments/ })).toBeNull()
  })
  ...
  it('says nothing when a review is still running', async () => {
    ...
    expect(screen.queryByRole('button', { name: /Post \d+ comment/ })).toBeNull()
  })
})
...
describe('posting individual comments', () => {
  ...
  it('offers to post one finding on its own', async () => {
    ...
    await openFindings()
    expect(await screen.findByRole('button', { name: /Post this finding on src\/jar\.py/ }))
      .toBeTruthy()
  })
  ...
  it('sends only that finding, keyed to its change', async () => {
    await openFindings()
    await userEvent.click(await screen.findByRole('button', {
      name: /Post this finding on src\/jar\.py/,
    }))
  ...
  it('marks a finding already on the pull request as posted', async () => {
    ...
    expect(screen.queryByRole('button', { name: /Post this finding on src\/jar\.py/ })).toBeNull()
    expect(screen.getByRole('button', { name: /Post this finding on src\/lid\.py/ })).toBeTruthy()
  })
})
...
describe('posting a chosen subset', () => {
  ...
  it('keeps the ticks when the grouped post is refused', async () => {
    ...
    await openFindings()
    await userEvent.click(screen.getByRole('checkbox', { name: /Select security on src\/jar\.py/ }))
    await userEvent.click(screen.getByRole('checkbox', { name: /Select style on src\/lid\.py/ }))
    await userEvent.click(await screen.findByRole('button', { name: /Post 2 selected/ }))
    ...
    expect(await screen.findByRole('button', { name: /Post 2 selected/ })).toBeTruthy()
  })
  ...
  it('clears the ticks once the grouped post succeeds', async () => {
    ...
    await openFindings()
    await userEvent.click(screen.getByRole('checkbox', { name: /Select security on src\/jar\.py/ }))
    await userEvent.click(await screen.findByRole('button', { name: /Post 1 selected/ }))

    await waitFor(() => expect(mockApi.postCommentGroups).toHaveBeenCalledTimes(1))
    await waitFor(() =>
      expect(screen.queryByRole('button', { name: /Post 1 selected/ })).toBeNull())
  })
  ...
  it('replaces post-all while a selection is live', async () => {
    await openFindings()
    expect(screen.getByRole('button', { name: /Post \d+ comments/ })).toBeTruthy()
    await userEvent.click(screen.getByRole('checkbox', { name: /Select security on src\/jar\.py/ }))
    // Offering "post all" beside "post 1 selected" invites sending more than
    // was chosen.
    expect(screen.queryByRole('button', { name: /Post \d+ comments/ })).toBeNull()
  ...
  it('untickes on a second click', async () => {
    await openFindings()
    const box = screen.getByRole('checkbox', { name: /Select security on src\/jar\.py/ })
    await userEvent.click(box)
    await userEvent.click(box)
    expect(screen.queryByRole('button', { name: /Post \d+ selected/ })).toBeNull()
  })
  ...
  it('offers no checkbox for a comment already posted', async () => {
    ...
    expect(screen.queryByRole('checkbox', { name: /Select security on src\/jar\.py/ })).toBeNull()
    expect(screen.getByRole('checkbox', { name: /Select style on src\/lid\.py/ })).toBeTruthy()
  })
})
...
describe('the ship-readiness summary', () => {
  ...
  it('shows the exact body that would be posted', async () => {
    ...
    expect(screen.getByText(/posted as a top-level comment/)).toBeTruthy()
  })

  it('can be posted on its own', async () => {
    await openRow()
    await userEvent.click(await screen.findByRole('button', {
      name: /Post the ship-readiness summary/,
    }))
    await waitFor(() => expect(mockApi.postComments).toHaveBeenCalledWith(
  ...
  it('says nothing when the report predates the ship body', async () => {
    ...
    expect(screen.queryByText(/posted as a top-level comment/)).toBeNull()
  })
})
